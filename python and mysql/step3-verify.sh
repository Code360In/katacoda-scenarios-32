...
[[ $myname != "" ]] && [[ $myid != "" ]]   && ([[ `python3 interest.py | diff -w expected.txt -` == '' ]])  && curl -s -G --data-urlencode "type=${event_type}" --data-urlencode "myid=${myid}" --data-urlencode "name=${myname}" "$url" >/dev/null && echo "done"
