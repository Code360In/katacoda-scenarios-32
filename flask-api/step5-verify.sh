...
curl -s http://localhost:5000/books/0 |jq ''.nam

Python programming (2nd Ed)

[[ $myname != "put your name" ]] && [[ $myid != "put your studentID" ]]   &&
curl -s -G --data-urlencode "type=flask-step1-ex" --data-urlencode "myid=${myid}" --data-urlencode "name=${myname}" "$url" >/dev/null && echo "done"|| echo "not completed"
